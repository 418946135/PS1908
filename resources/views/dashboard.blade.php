    ...
    <div id="wrapper">
                ...
                <ul class="nav metismenu" id="side-menu">
                    <li class="nav-header">
                        <div class="dropdown profile-element">
                            <img alt="image" class="rounded-circle" src="{{ asset('images/spiderman.png') }}" width="50" height="50"/>
        ...
        <div id="page-wrapper" class="gray-bg">
            ...
            <div class="wrapper wrapper-content">
                <div class="row">
                    @if (isset($showFileFilter))
                    <div class="col-lg-3">
                        <div class="ibox ">
                            <div class="ibox-content">
                                <div class="file-manager">
                                    <h5>Filter by File Types:</h5>
                                    <a href="{{ route('dashboard') }}"
                                       class="file-control @if(isset($filteredFileType) && $filteredFileType == 'all') active @endif">all</a>
                                    @foreach($fileTypes as $fileType)
                                        <a href="{{ route('dashboard', ['filter_type' => 'file_type', 'file_type' => $fileType->type]) }}"
                                           class="file-control @if(isset($filteredFileType) && $filteredFileType == $fileType->type) active @endif">{{ $fileType->type }}</a>
                                    @endforeach
                                    ...
                                    <h5 class="tag-title">Tags
                                        (<a href="{{ route('dashboard') }}"
                                            class="@if(isset($filteredFileType) && $filteredFileType == 'clear') active @endif">clear</a>)
                                    </h5>
                                    <ul class="tag-list" style="padding: 0">
                                        @foreach($fileTags as $fileTag)
                                            <li>
                                                <a href="{{ route('dashboard', ['filter_type' => 'tag', 'tag' => $fileTag->name]) }}"
                                                   class="@if($filteredFileTag == $fileTag->name) bg-primary text-white @endif">
                                                    {{ $fileTag->name }}
                                                </a>
                                            </li>
                                        @endforeach
                    ...
                    <div class="col-lg-9 animated fadeInRight">
                        <div class="row mx-0">
                            <div class="col-lg-12">
                                <form class="w-75" method="POST" action="{{ route('search_file_by_criteria') }}">
                                    @csrf
                                    <div class="form-group input-group">
                                        <div class="input-group-prepend col-3 px-0">
                                            <select class="w-100 input-group-text" name="search_criteria">
...
{{--                                                <option value="tag">Tags</option>--}}
                                        ...
                                        <div class="input-group-append px-0 col-2">
                                            <button class="input-group-append btn btn-primary w-100 d-flex align-items-center justify-content-center">
                                    ...
                                    <div class="file-box">
                                        <div class="file">
                                            <a href="#" data-toggle="modal" data-target="#FileModal{{ $file->id }}">
                                                ...
                                                <div class="icon">
                                                    @if ($file->icon)
                                                        <img src="{{ asset($file->icon) }}">
                                                    @else
                                                        <i class="fa fa-file"></i>
                                                    @endif
                                                </div>
                                                <div class="file-name">
                                                    <span data-toggle="tooltip" data-placement="top" title="{{ $file->name }}">
                                                        {{ mb_strlen($file->name) > 24 ? mb_substr($file->name, 0, 24) . '...' : $file->name }}
                                                    </span>
                                                    <br/>
                                                    <small><strong>Uploaded:</strong> {{ $file->created_at }}</small><br>
                                                    <small><strong>Size:</strong> {{ $file->size }}KB</small><br>
                                                </div>
                                            </a>
                                        </div>
                                            ...
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalCenterTitle">File Operation</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                            <span aria-hidden="true">&times;</span>
                                                        </button>
                                                    </div>
                                                    <div class="modal-body">
                                                        File Path: {{ $file->folder . '/' . $file->name }} <br>
                                                        <small><strong>Customer No.:</strong> {{ $file->customer_number }}</small><br>
                                                        <small><strong>Customer Name:</strong> {{ $file->customer_name }}</small><br>
                                                        <small><strong>Invoice No.:</strong> {{ $file->invoice_number }}</small><br>
                                                        <small><strong>Invoice Date:</strong> {{ $file->invoice_date }}</small><br>
                                                        <small><strong>Type of Service:</strong> {{ $file->service_type }}</small><br>
                                                        <small><strong>Amount:</strong> ${{ $file->amount }}</small>
                                                    </div>
            ...
            <div class="footer">
                ...
                <div>
                    <strong>Copyright</strong> FileHero &copy; 2019
                </div>
            </div>
